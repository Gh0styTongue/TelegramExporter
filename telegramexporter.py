...
# Wrapper to make input() non-blocking for Pyrogram keepalives
...
async def get_chat_logs_by_identifier(client_instance, chat_identifier, directory_name):
    partecipants_ids = list()

    try:
        # Changed iter_chat_members to get_chat_members (async)
        async for members in client_instance.get_chat_members(chat_identifier):
            partecipants_ids.append(members.user.id)
    except Exception as e:
        if "ChatParticipantsForbidden" in str(e):
    ...
    with open("configuration.json", "r") as json_config:
    ...
    while chat_obj is None:
        try:
            # Added await
            chat_obj = await client_instance.get_chat(chat_identifier)
        except FloodWait:
    ...
    chat_title = ""
    if chat_obj.type.value == "channel": # v2 uses enums, checking value string or accessing property
        if chat_obj.username is not None:
            chat_title = chat_obj.username
        else:
            chat_title = chat_obj.title
    elif str(chat_obj.type) == "ChatType.CHANNEL": # Safety check for enum string
         if chat_obj.username is not None:
            chat_title = chat_obj.username
         else:
            chat_title = chat_obj.title
    ...
    while True:
        try:
            ...
            # Changed iter_history to get_chat_history (async)
            async for message in client_instance.get_chat_history(chat_identifier):
                chat.append(message)
            ...
            for msg in chat:
                if export_media == 1:
                    if msg.media:
                        try:
                            ...
                            # Added await for download_media
                            await client_instance.download_media(msg, file_name=create_path)
                        except ValueError:
                            print(
                ...
                # Handling date (v2 date is already datetime object)
                date_val = msg.date
                if isinstance(date_val, int):
                    _formatted_message_date = datetime.utcfromtimestamp(date_val).strftime(_TIME_FORMAT)
                else:
                    _formatted_message_date = date_val.strftime(_TIME_FORMAT)
...
async def get_contact(client_instance, targets=None):
    if targets is None:
        targets = []
    saved_contact = list()
    ...
    for target in targets:
        # Changed iter_dialogs to get_dialogs (async)
        async for dialog in client_instance.get_dialogs():
            # In v2, chat.type is an Enum. Convert to str for comparison
            d_type = str(dialog.chat.type).lower() # e.g. "chattype.private"
            
            # Simple check for types
            is_private = "private" in d_type
            is_bot = "bot" in d_type
            
            if is_private or is_bot:
                # get_users is async
                users_list = await client_instance.get_users([dialog.chat.id])
                user = users_list[0] if isinstance(users_list, list) else users_list
                ...
                if title and target in title.lower():
                    ...
                    non_contact_type_dict[dialog.chat.id] = str(dialog.chat.type)

    return saved_contact, non_contact_chat_dict, non_contact_type_dict


async def menu_get_contact(client_instance):
    target_name = await async_input("You can enter one of the following information: "
                        "\n- Phone Book name \n- Telegram username \n- Channel name \n- Group name "
    ...
    users, non_user_dict, non_contact_type_dict = await get_contact(client_instance, [target_name.lower()])
    ...
    if key < len(users):
        chat = await client_instance.get_chat(users[key].id)
        return users[key].id, str(chat.type)
    else:
        chat_id = list(non_user_dict)[key - len(users)]
        return chat_id, non_contact_type_dict[chat_id]


async def menu_get_multiple_contact(client_instance):
    target_name = str(await async_input("User separator ';' to select multiple name.\n"
                            "Enter your decision: "))
    ...
    if target_name.__contains__(";"):
        ...
        users, non_user_dict, non_contact_type_dict = await get_contact(client_instance, users_split)
    else:
        print("Please, use ;")
    ...
    for user in users:
        ...
        key += 1
        ids.append(user.id)
        chat = await client_instance.get_chat(user.id)
        types_dict[user.id] = str(chat.type)
...
async def get_multiple_chat_ids_by_dialogs(client_instance, multiple_ids_chats):
    chat_ids_list = list()
        ...
        async for dialog in client_instance.get_dialogs():
            if dialog.chat.id == ids_chats:
                if dialog.chat.username is not None:
                    ...
                    user_obj_list = await client_instance.get_users(ids)
...
async def get_chat_ids_by_dialogs(client_instance, single_chat_id=None):
    chat_ids_list = list()
    chat_id_usernames_dict = dict()
    chat_id_title_dict = dict()
    chat_id_full_name_dict = dict()
    ...
    async for dialog in client_instance.get_dialogs():
        if (single_chat_id is None) or (single_chat_id is not None and dialog.chat.id == single_chat_id):
            ...
            chat_type_dict[dialog.chat.id] = str(dialog.chat.type)
...
async def write_all_chats_logs_file(client_instance, chat_ids_list, chat_id_usernames_dict, chat_id_title_dict,
    ...
    for chat_id in chat_ids_list:
        ...
        # Async call
        log_lines, partecipants_ids = await get_chat_logs_by_identifier(client_instance, chat_id, directory_name)
        ...
        if partecipants_ids:
            ...
            with open(saving_file_path, "w", encoding="UTF-16") as file:
                file.write(header + "\n")
                # get_users is async
                users_list = await client_instance.get_users(partecipants_ids)
                # It might return a single user if list len is 1, ensure iterable
...
async def write_group_chats_members(client_instance, chat_title_list):
    for chat_id in chat_title_list:
        title = chat_title_list[chat_id]
        list_username = list()
        try:
            # get_chat_members is async
            async for member in client_instance.get_chat_members(chat_id):
...
async def main():
    ...
    try:
        my_api_id = config.get("pyrogram", "api_id").strip()
        my_api_hash = config.get("pyrogram", "api_hash").strip()
    except Exception as e:
        print(f"{classes.BColor.FAIL}Error reading config.ini. Make sure it exists and has [pyrogram] section with api_id and api_hash.{classes.BColor.ENDC}")
        return

    if os.path.exists("extraction"):
        clean_folder = await async_input("Do you want to clean extraction folder from previous extractions files? (y/N): ")
        if clean_folder == 'y':
    ...
    while response != 0:
        ...
        # Keep client open for the whole session loop
        async with Client("my_account", api_id=my_api_id, api_hash=my_api_hash, hide_password=True) as client:
            try:
                # Use async_input to prevent loop blocking
                choice_str = await async_input("\nEnter: \n[1] to extract the chats for a single user "
                                               "         \n[2] to extract the chats for multiple users"
                                               "         \n[3] to extract all chats"
                                               "         \n[-1] to quit"   
                                               "         \nPlease enter your choice: ")
                type_of_extraction = int(choice_str)

                if type_of_extraction == 1:
                    create_extraction_folders()
                    chatId, chat_type = await menu_get_contact(client)
                    chatIdsList, chatIdUsernamesDict, chatIdTitleDict, chatIdFullNameDict, deletedChatIds, chatIdPhoneNumberDict, chat_type_list = await get_chat_ids_by_dialogs(
                        client, chatId)
                    await write_all_chats_logs_file(client, chatIdsList, chatIdUsernamesDict, chatIdTitleDict,
                                              chatIdFullNameDict, deletedChatIds, chatIdPhoneNumberDict, chat_type)
                    compress_and_hash_extraction()

                elif type_of_extraction == 2:
                    create_extraction_folders()
                    chatIds, chat_types = await menu_get_multiple_contact(client)
                    chatIdsList, chatIdUsernamesDict, chatIdTitleDict, chatIdFullNameDict, chatIdPhoneNumberDict = \
                        await get_multiple_chat_ids_by_dialogs(client, chatIds)
                    await write_all_chats_logs_file(client, chatIdsList, chatIdUsernamesDict, chatIdTitleDict,
                    ...
                    chatIdPhoneNumberDict, chat_type_dict = await get_chat_ids_by_dialogs(client)
                    await write_all_chats_logs_file(client, chatIdsList, chatIdUsernamesDict, chatIdTitleDict, chatIdFullNameDict,
                ...
                if not "No contacts found" in str(e):
                    print(str(e))
                # Break loop on critical error to prevent infinite restart loop
